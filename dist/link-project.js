...
// src/link-project.ts
async function linkProjectAction() {
  try {
    const owner = core2.getInput("owner", { required: true });
    const projectNumber = core2.getInput("project-number", { required: true });
    const repository = core2.getInput("repository");
    const team = core2.getInput("team");
    const linked = core2.getBooleanInput("linked");
    if (!repository && !team) {
      core2.setFailed("Input required and not supplied: repository or team");
      return;
    }
